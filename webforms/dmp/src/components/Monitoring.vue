  ...
  <div v-if="info">
    <div class="question-wrapper">
      ...
      <div style="margin-top: 20px;">
        <b-alert show variant="info"><p v-html="info.data.table_note"></p></b-alert>
      </div>
      ...
      <div class="answer">
        ...
        <div style="position: relative" role="tablist">
          <b-card v-for="(article,index) in info.data.articles" class="mb-1">
            ...
            <b-collapse class="mt-3" :id="`article_${index}`" accordion="my-accordion" role="tabpanel">
              ...
              <div class="form-fields mb-3" v-for="(item, array_index) in article.article_items">
                ...
                <b-form-input class="form-control" required v-if="item.type === 'text'"
                              :id="`${tabId}_${index}_${array_index}_${item.name}_${item.type}`" :type="item.type"
                              :name="item.name" v-model="item.selected"></b-form-input>

                <b-form-radio-group required stacked v-if="item.type === 'radio'"
                                    :id="`${tabId}_${index}_${array_index}_${item.name}_${item.type}`" :type="item.type"
                                    :name="item.name" v-model="item.selected"
                                    :options="item.options"></b-form-radio-group>

                <b-form-checkbox-group stacked v-if="item.type === 'checkbox'"
                                       :id="`${tabId}_${index}_${array_index}_${item.name}_${item.type}`"
                                       :type="item.type" :name="item.name" v-model="item.selected"
                                       :options="item.options"></b-form-checkbox-group>
          ...
          <b-btn style="
                  ...
                  top: -3rem;
                  right: 13px;"
                 variant="primary" @click="addDump"> + Add
          </b-btn>
        </div>
      </div>
