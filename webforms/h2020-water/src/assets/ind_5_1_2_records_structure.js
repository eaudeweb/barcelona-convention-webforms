...
const ind_5_1_2_records_structure = (data) => ({
  nationalStationID: {
  ...
  samplingTime: {
    ...
    type: 'text',
    selected: data && data.samplingTime || null,
    get validation() {
      if (!this.selected)
        return `${this.label} is required`
    }
  ...
  sampleID: {
    ...
    tooltip: 'Use the former sampling guidelines provided by UNEP-MAP',
    type: 'text',
    selected: data && data.sampleID || null,
    get validation() {
      if (!this.selected)
        return `${this.label} is required`
    }
  ...
  determinantNutrients: {
    ...
    type: 'select',
    selected: data && data.determinantNutrients || null,
  ...
  unitNutrientsSeawater: {
    ...
    type: 'select',
    selected: data && data.unitNutrientsSeawater || null,
    options: water_collection_method_i.map(p => ({text: `${p.text}`, value: p.value})),
    get validation() {
      return true
    }
  ...
  concentrationFlag: {
    ...
    type: 'text',
    selected: data && data.concentrationFlag || null,
    get validation() {
      return true
    }
  },
  concentration: {
    ...
    type: 'number',
    selected: data && data.concentration || null,
    get validation() {
  ...
  concentrationStatusUnit: {
    ...
    tooltip: 'Use the former sampling guidelines provided by UNEP-MAP',
    type: 'text',
    selected: data && data.concentrationStatusUnit || null,
    get validation() {
      return true
    }
  },
  sampleDepth: {
    name: 'sampleDepth',
    type: 'number',
    selected: data && data.sampleDepth || null,
